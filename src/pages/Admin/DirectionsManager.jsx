...
const DirectionsManager = () => {
    ...
    const handleImageUpload = async () => {
        ...
        const imageRef = ref(storage, `directions/${Date.now()}_${imageFile.name}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingId) {
                const itemDoc = doc(db, 'directions', editingId);
                await updateDoc(itemDoc, { ...formData, imageUrl: finalImageUrl });
            } else {
                await addDoc(directionsCollectionRef, { ...formData, imageUrl: finalImageUrl });
            }
            setModalOpen(false);
            fetchDirections();
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Rostdan ham o'chirmoqchimisiz?")) {
            try {
                const itemDoc = doc(db, 'directions', id);
                    ...
                    <table className="crud-table">
                        ...
                        <tbody>
                            {directions.map((n) => (
                                <tr key={n.id}>
                                    <td>
                                        {n.imageUrl ? <img src={n.imageUrl} alt={n.title} style={{ width: '60px', height: '40px', objectFit: 'cover', borderRadius: '4px' }} /> : 'Yo\'q'}
                                    </td>
                                    <td>{n.title}</td>
                                    <td>
